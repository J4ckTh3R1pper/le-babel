...
public interface CategoryRepository extends JpaRepository<PostCategory, Integer>, JpaSpecificationExecutor<PostCategory> {
    ...
    @Query("""
        SELECT new dayp308.lebabel.bean.entity.projection.CategoryProjection(
            c.id,
            c.name,
            c.createTime,
            c.avatar,
        ...
        LEFT JOIN CategoryMember AS m ON m.category = c AND m.role <> dayp308.lebabel.enumeration.Role.GUEST
        WHERE c.id = :id
    """)
